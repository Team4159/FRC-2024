...
public class SpeakerLookupTable extends ParallelCommandGroup {
    ...
    public SpeakerLookupTable(Kinesthetics k, CommandSwerveDrivetrain sw, Shooter sh, Neck n,DoubleSupplier translationSup, DoubleSupplier strafeSup){
        this(k, sw, sh, n, translationSup, strafeSup, false);
    }
    public SpeakerLookupTable(Kinesthetics k, CommandSwerveDrivetrain sw, Shooter sh, Neck n, DoubleSupplier translationSup, DoubleSupplier strafeSup, boolean continuous){
        double rootg = Math.sqrt(Constants.Environment.G);
        SwerveRequest.FieldCentricFacingAngle yawRequest = new SwerveRequest.FieldCentricFacingAngle();
        addCommands(
            ...
            new SequentialCommandGroup(
                sh.new ChangeState(() -> new ShooterCommand(bestPitch(getDifference(sw).toTranslation2d().getNorm()), 350d, 250d), continuous),
    ...
    public SpeakerLookupTable(Kinesthetics k, CommandSwerveDrivetrain sw, Shooter sh, Neck n, DoubleSupplier translationSup, DoubleSupplier strafeSup, double timeout){
        double rootg = Math.sqrt(Constants.Environment.G);
    ...
    private static Translation3d getDifference(CommandSwerveDrivetrain sw) {
        var all = DriverStation.getAlliance();
        if (all.isEmpty()) return null;
        var t2 = sw.getPose().getTranslation();
